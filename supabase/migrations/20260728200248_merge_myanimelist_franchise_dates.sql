...
begin
  ...
  loop
    ...
    begin
      ...
      v_start_date := nullif(v_item ->> 'startDate', '')::date;
      v_finish_date := nullif(v_item ->> 'finishDate', '')::date;
    exception
    ...
    if v_start_date is null and v_finish_date is not null then
      v_start_date := v_finish_date;
    elsif v_start_date is not null
      and v_finish_date is not null
      and v_finish_date < v_start_date
    then
      v_skipped := v_skipped + 1;
      continue;
    end if;
    ...
    set status = excluded.status,
        started_at = least(
          public.user_library.started_at,
          public.user_library.finished_at,
          excluded.started_at,
...
comment on function public.import_myanimelist_batch(jsonb) is
  'Atomically imports a reviewed MyAnimeList selection, merging franchise dates as the earliest known start and latest known finish while preserving per-entry progress.';
